...
class PPO(BaseAlgo):
    ...
    def __init__(self, env: BaseTask, config, log_dir=None, device='cpu'):
        ...
        self.writer = TensorboardSummaryWriter(log_dir=self.log_dir,
                                               flush_secs=10)
        ...
        self.cur_reward_sum = torch.zeros(self.env.num_envs,
                                          dtype=torch.float,
                                          device=self.device)
    ...
    def _setup_models_and_optimizer(self):
        self.actor = PPOActor(obs_dim_dict=self.algo_obs_dim_dict,
                              module_config_dict=self.config.module_dict.actor,
                              num_actions=self.num_act,
                              init_noise_std=self.config.init_noise_std).to(
                                  self.device)
    ...
    def _setup_storage(self):
        ...
        ## Register others
        self.storage.register_key('actions',
                                  shape=(self.num_act, ),
                                  dtype=torch.float)
        ...
        self.storage.register_key('actions_log_prob',
                                  shape=(1, ),
                                  dtype=torch.float)
        self.storage.register_key('action_mean',
                                  shape=(self.num_act, ),
                                  dtype=torch.float)
        self.storage.register_key('action_sigma',
                                  shape=(self.num_act, ),
                                  dtype=torch.float)
        ...
        if ckpt_path is not None:
            ...
            if self.load_optimizer:
                ...
                self.actor_learning_rate = loaded_dict[
                    'actor_optimizer_state_dict']['param_groups'][0]['lr']
                self.critic_learning_rate = loaded_dict[
                    'critic_optimizer_state_dict']['param_groups'][0]['lr']
                self.set_learning_rate(self.actor_learning_rate,
    ...
    def learn(self):
        ...
        self.save(
            os.path.join(self.log_dir, 'model_{}.pt'.format(
    ...
    def _compute_returns(self, last_obs_dict, policy_state_dict):
        ...
        for step in reversed(range(num_steps)):
            ...
            delta = rewards[
                step] + next_is_not_terminal * self.gamma * next_values - values[
                    step]
    ...
    def _post_epoch_logging(self, log_dict, width=80, pad=35):
        ...
        with Live(Panel(log_string, title="Training Log"),
    ...
    def _logging_to_writer(self, log_dict, train_log_dict, env_log_dict):
        ...
        for loss_key, loss_value in log_dict['loss_dict'].items():
            self.writer.add_scalar(f'Loss/{loss_key}', loss_value,
                                   log_dict['it'])
        ...
        self.writer.add_scalar('Perf/total_fps', train_log_dict['fps'],
                               log_dict['it'])
        self.writer.add_scalar('Perf/collection time',
                               log_dict['collection_time'], log_dict['it'])
        self.writer.add_scalar('Perf/learning_time', log_dict['learn_time'],
                               log_dict['it'])
        if len(log_dict['rewbuffer']) > 0:
    ...
    def evaluate_policy(self):
        ...
        obs_dict = self.env.reset_all()
        init_actions = torch.zeros(self.env.num_envs,
                                   self.num_act,
                                   device=self.device)
        ...
        if self.config.eval_callbacks is not None:
            for cb in self.config.eval_callbacks:
                self.eval_callbacks.append(
                    instantiate(self.config.eval_callbacks[cb],
                                training_loop=self))
